...
sql <- "SELECT bp.bookid, round(avg(length(bp.content))) as avgLength, round(STD(length(bp.content))) as stdLength 
            FROM bookContent bp  
            GROUP BY bookid;"
rs <- dbGetQuery(con, sql)
return(rs)
...
sql <- "SELECT bl.bookid, round(avg(length(bl.content))) as avgLength, round(STD(length(bl.content))) as stdLength 
...
You can start trying `-country +president` in boolean mode.
...
```{r, echo=FALSE}
        ...
        server <- function(input, output, session) {
                ...
                #output$tableStat <- renderTable({DataStat()})
